...
MONGODB_COLLECTION = "clean_air_race_pace"
UNIQUE_KEY = "clean_air_race_pace_id"
...
def process_race_pace_data(event_data: pd.DataFrame, season: int = 2025) -> pd.DataFrame:
    logger.info(f"Processing clean_air_race_pace data.")
    ...
    for _, event in event_data.iterrows():
        ...
        # Calculate clear air laps
        # Sort all accurate laps by time to find the gap to the car ahead
        all_laps_sorted = laps.sort_values(by="Time")
        all_laps_sorted["GapToAhead"] = all_laps_sorted["Time"].diff().dt.total_seconds()
        
        # The first car in the sequence (leader on track) has infinite gap
        all_laps_sorted["GapToAhead"] = all_laps_sorted["GapToAhead"].fillna(999999)

        for driver in drivers:
            curr_driver_laps = all_laps_sorted[all_laps_sorted["Driver"] == driver]
            
            # Clean Air Definition: Gap > 2.0s
            clean_air_laps = curr_driver_laps[curr_driver_laps["GapToAhead"] > 2.0]
            
            # General stats (for comparison)
            avg_laptime = curr_driver_laps["LapTime"].mean()
            std_laptime = curr_driver_laps["LapTime"].std()
            
            # Clean Air stats
            avg_laptime_clean_air = clean_air_laps["LapTime"].mean()
            std_laptime_clean_air = clean_air_laps["LapTime"].std()
            ...
            all_race_pace.append({
                ...
                "avg_laptime_clean_air": avg_laptime_clean_air.total_seconds() if pd.notnull(avg_laptime_clean_air) else None,
                "std_laptime_clean_air": std_laptime_clean_air.total_seconds() if pd.notnull(std_laptime_clean_air) else None,
                "clean_air_laps_count": len(clean_air_laps),
                "clean_air_race_pace_id": f"{season}_{round_number}_{driver}"
            })
    ...
    # Fill NaNs before casting to int, as rank() returns NaN for missing values.
    df_race_pace["clean_air_race_pace_position"] = (
        df_race_pace.groupby(["season", "round"])["avg_laptime_clean_air"]
        .rank(method="min", ascending=True)
        .fillna(-1)
        .astype(int)
    )
    
    # Replace the placeholder -1 with None for the database
    df_race_pace["clean_air_race_pace_position"] = df_race_pace["clean_air_race_pace_position"].replace(-1, None)

    logger.info(f"Processed {len(all_race_pace)} clean air race pace records.")
